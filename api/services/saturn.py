"""
Saturn position calculation service.

This module provides Saturn position calculations using shared common_bodies
...
def calculate_saturn_position(
    ...
    Returns:
        Dictionary containing:
            ...
            - azimuth: Saturn's azimuth in degrees (0=North, 90=East, 180=South, 270=West)
            - is_visible: Boolean indicating if Saturn is above horizon
            - ra_degrees: Saturn's right ascension in degrees (topocentric, observer-dependent)
            - dec_degrees: Saturn's declination in degrees (topocentric, observer-dependent)
            - retrograde_status: Whether Saturn is in retrograde motion from Earth's
    ...
    """
    return calculate_outer_planet_position(observation_time, location, "saturn", locale)


def _process_saturn_position(
    saturn_with_loc,
    saturn_altaz,
    saturn_gcrs,
    time,
    ...
    """
    Process Saturn position data into response format.
    Internal function used by batch operations.

    Args:
        saturn_with_loc: Saturn position in topocentric GCRS frame
        saturn_altaz: Saturn position in AltAz frame
        saturn_gcrs: Saturn position (GCRS coordinates) - used for retrograde
    ...
    Returns:
        Dictionary with Saturn position data
    """
    # Format base position data
    position_dict = _process_body_position(
        saturn_altaz, saturn_with_loc, time, datetime_str, location
    )

    # Add retrograde status (pre-computed in batch, calculated on-demand)
    if retrograde_status is None:
        retrograde_status = get_retrograde_status(saturn_gcrs, time, "saturn")
