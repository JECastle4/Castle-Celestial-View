...
| K7 | With pin mode active, use **Arrow keys** to pan (and **+** / **−** to zoom) until the crosshair is over the desired location, then press **Enter**. Alternatively, click anywhere on the map. | Pin is placed at the crosshair centre (or click point). Tool deactivates immediately: icon reverts, crosshair is removed, cursor resets. Screen reader announces *"Pin placed."* — **Cancel path:** press **Escape** or re-activate the **Place Pin** button; tool deactivates without placing a pin and screen reader announces *"Pin placed."* |
